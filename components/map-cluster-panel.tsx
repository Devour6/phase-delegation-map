...
export function MapClusterPanel({
  ...
  const isSearchMode = searchValue != null && onSearchChange != null;
  ...
  // Auto-focus search input when panel opens in search mode
  useEffect(() => {
  ...
  if (validators.length === 0 && !isSearchMode) return null;
    ...
    <>
      ...
      <div className="fixed right-0 top-[52px] bottom-0 z-40 flex w-full max-w-sm flex-col overflow-hidden border-l border-border bg-background/95 backdrop-blur-xl md:w-96">
        ...
        <div className="border-b border-border px-6 py-4">
          {isSearchMode && (
            <input
              ...
              className="font-body mb-3 w-full rounded-lg border border-border bg-background/70 px-3 py-2 text-xs text-foreground placeholder:text-muted backdrop-blur-md focus:border-accent focus:outline-none"
            />
          )}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          {validators.length === 0 ? (
            <p className="font-body py-8 text-center text-xs text-muted">
              No validators found
            </p>
          ) : (
